...
- **Latency is dominated by distance, not by code.** `/languages` (no DB) answers in 7ms; anything touching the database pays ~0.12s per round trip to `ap-southeast-1` and the queries largely serialise on a shared 0.25 vCPU. `perf-001` cut the round trips per request (`/student/diagnostic` 12 → 4); the remaining ~1s floor is the network and cannot be fixed from this repo.
